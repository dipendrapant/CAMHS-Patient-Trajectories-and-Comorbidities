...
class CoocurringDiagnosisNetworkGraph_higherlevel1:
    def load_plot_network_graph(self, original_df):
        ...
        for diagnosis in df.columns[2:]:
            high_level_diagnosis = diagnosis[:1]
            # Create a temporary sum column for each high-level diagnosis count
            if high_level_diagnosis not in df:
                df[high_level_diagnosis] = df[
                    [col for col in df.columns if col.startswith(high_level_diagnosis)]
                ].max(axis=1)
        ...
        for high_level_diagnosis in unique_high_level_diagnoses:
            count = df[(df["F90"] == 1) & (df[high_level_diagnosis] == 1)][
                "pasient"
            ].nunique()
            if count > 0:
                new_row = {
                    "F90Cooccuring": high_level_diagnosis,
                    "F90CooccuringValue": count,
                }
                new_rows.append(new_row)

        # Append all new rows at once
        df_coocurring = pd.concat(
            [df_coocurring, pd.DataFrame(new_rows)], ignore_index=True
        )
        print(df_coocurring["F90CooccuringValue"].sum())
