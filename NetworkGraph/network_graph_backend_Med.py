...
#             node_positions,
#             nodelist=[node for node in G.nodes() if node != "F90"],
#             node_size= 500, #[300 + 20*data[node] for node in G.nodes()], 
#             node_color="#60c3dd",
#         )
#         nx.draw_networkx_labels(G, node_positions, labels={"F90": "F90"}, font_size=20)
...
class CoocurringDiagnosisNetworkGraph_higherlevel11: # just having node size correlated to the co-occurrence
    def load_plot_network_graph(self, original_df):
        # Load CSV data
        original_df = original_df
        print(original_df.shape)
